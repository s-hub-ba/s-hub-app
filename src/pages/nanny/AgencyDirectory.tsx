...
import { Search, MapPin, ShieldCheck, Star, Zap, TrendingUp, ArrowRight } from 'lucide-react';
import { motion } from 'motion/react';
import { useAuth } from '../../contexts/AuthContext';
import {
  getAgencies,
  getNannyFollowedAgencies,
  followAgencyAsNanny,
  unfollowAgencyAsNanny,
} from '../../lib/api';
...
export default function NannyAgencyDirectory() {
  ...
  useEffect(() => {
    if (!user) return;
    getNannyFollowedAgencies(user.uid)
  ...
  const toggleFollow = async (agencyId: string) => {
    ...
    try {
      if (isFollowing) {
        await unfollowAgencyAsNanny(user.uid, agencyId);
        setFollowedAgencyIds(prev => prev.filter(id => id !== agencyId));
      } else {
        await followAgencyAsNanny(user.uid, agencyId);
  ...
  return (
    <div className="min-h-screen bg-stone-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-stone-900">Browse Agencies</h1>
          <p className="text-stone-500 mt-1">
            Follow agencies to see their updates in your dashboard feed.
          </p>
        </div>
