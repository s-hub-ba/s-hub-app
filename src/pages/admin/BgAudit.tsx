  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-stone-200 flex flex-col md:flex-row gap-3">
        ...
        <select
          value={actionFilter}
          onChange={(e) => setActionFilter(e.target.value as 'all' | 'created' | 'updated' | 'override_set' | 'override_cleared')}
          ...
          >
            ...
            <div className="px-6 pt-6 pb-4 space-y-5 max-h-[60vh] overflow-y-auto">
              <div>
                ...
                <select value={overrideStatus} onChange={(e) => setOverrideStatus(e.target.value as 'checked' | 'not_checked' | 'expired')} className="w-full bg-stone-50 border border-stone-200 rounded-2xl px-4 py-3 text-stone-800 font-medium focus:outline-none focus:ring-2 focus:ring-violet-400 transition">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" min={0} max={100} value={overrideConfidence} onChange={(e) => setOverrideConfidence(e.target.value)} className="w-full bg-stone-50 border border-stone-200 rounded-2xl px-4 py-3 text-stone-800 focus:outline-none focus:ring-2 focus:ring-violet-400 transition" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-stone-400 uppercase tracking-widest mb-1.5">Expires At</label>
                  <input type="date" value={overrideExpiresAt} onChange={(e) => setOverrideExpiresAt(e.target.value)} className="w-full bg-stone-50 border border-stone-200 rounded-2xl px-4 py-3 text-stone-800 focus:outline-none focus:ring-2 focus:ring-violet-400 transition" />
                </div>
              </div>

              <div>
                <label className="block text-xs font-bold text-stone-400 uppercase tracking-widest mb-1.5">Reason</label>
                <textarea value={overrideReason} onChange={(e) => setOverrideReason(e.target.value)} rows={4} className="w-full bg-stone-50 border border-stone-200 rounded-2xl px-4 py-3 text-stone-800 placeholder-stone-400 focus:outline-none focus:ring-2 focus:ring-violet-400 resize-none transition" placeholder="Why is this override needed?" />
              </div>
            </div>
