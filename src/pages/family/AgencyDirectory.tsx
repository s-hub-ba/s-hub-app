...
import { Search, MapPin, ShieldCheck, Star, Zap, TrendingUp, ArrowRight, Send } from 'lucide-react';
...
export default function FamilyAgencyDirectory() {
  ...
  useEffect(() => {
    if (!user) return;
    getFamilyFollowedAgencies(user.uid)
      .then(ids => setFollowedAgencyIds(ids || []))
      .catch(() => {});
  }, [user]);
  ...
  return (
    <div className="min-h-screen bg-stone-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-stone-900">Find an Agency</h1>
          <p className="text-stone-500 mt-1">
            Browse trusted, vetted nanny agencies in New York City. Send an inquiry or follow agencies to stay updated.
          </p>
        </div>

        {/* Featured */}
        {agencies.some(a => a.isSponsored || a.sponsored) && (
          <div className="mb-10">
            <div className="flex items-center gap-2 mb-4">
              <Star className="h-5 w-5 text-amber-500 fill-current" />
              <h2 className="text-lg font-bold text-stone-900">Featured Agencies</h2>
            </div>
                ...
                >
                  ...
                  <div className="flex items-center gap-4 mb-4">
                    {agency.logo ? (
                      ...
                      <div className="w-16 h-16 rounded-2xl border-2 border-white shadow-sm bg-stone-100 flex items-center justify-center text-stone-600 font-bold text-xl">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <Link
                      ...
                      className="inline-flex items-center text-sm font-semibold text-emerald-600 hover:text-emerald-700 ml-auto"
                    >
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {filteredAgencies.map((agency, index) => (
            <motion.div
              ...
              className={`bg-white rounded-2xl border overflow-hidden hover:shadow-md transition-shadow flex flex-col relative ${
            ...
            >
              ...
              <div className="p-6 flex-1">
                <div className="flex items-start gap-4 mb-4">
                  {agency.logo ? (
                    ...
                    <div className="w-16 h-16 rounded-xl border border-stone-100 bg-stone-100 flex items-center justify-center text-stone-600 font-bold text-xl">
                      {(agency.company_name || agency.name || 'A').charAt(0).toUpperCase()}
                    </div>
                  )}
                  <div>
                    ...
                    {agency.plan_tier && agency.plan_tier !== PLAN_CODES.STARTER && (
                      <span
                        className={`inline-flex items-center gap-1 mt-1.5 px-2 py-0.5 rounded-full text-xs font-semibold ${
                          TIER_CONFIG[agency.plan_tier]?.className ?? ''
                        }`}
                      >
              ...
              <div className="p-4 border-t border-stone-100 bg-stone-50/50 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  >
                    {followedAgencyIds.includes(agency.id) ? 'Following ✓' : 'Follow'}
                  </button>
                  <Link
                    to="/family/request-care"
                    className="flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl text-sm font-medium bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-200 transition-colors"
                  >
